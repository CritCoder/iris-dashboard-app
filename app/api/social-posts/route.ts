...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('📊 SOCIAL POSTS - Proxying request to:', fullUrl);
    ...
    console.log('📥 SOCIAL POSTS - Response status:', response.status);
    ...
    if (!response.ok) {
      ...
      console.error('❌ SOCIAL POSTS - Error response:', error);
      return NextResponse.json(error, { status: response.status });
    }

    const data = await response.json();
    console.log('✅ SOCIAL POSTS - Success:', {
      hasData: !!data.data,
    ...
    console.error('💥 SOCIAL POSTS - API Proxy Error:', error);
