...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = `${API_BASE_URL}/api/campaigns/search${queryString ? `?${queryString}` : ''}`;

    console.log('🔍 CAMPAIGNS SEARCH - Proxying request to:', url);
    ...
    console.log('📥 CAMPAIGNS SEARCH - Response status:', response.status);
    ...
    if (!response.ok) {
      ...
      if (contentType?.includes('application/json')) {
        ...
        error = { message: text || 'Failed to search campaigns', status: response.status };
      }
      console.error('❌ CAMPAIGNS SEARCH - Error response:', error);
      return NextResponse.json(error, { status: response.status });
    }

    const data = await response.json();
    console.log('✅ CAMPAIGNS SEARCH - Success:', {
      hasData: !!data.data,
    ...
    console.error('💥 CAMPAIGNS SEARCH - API Proxy Error:', error);
