...
// Extract unique API endpoints from responses (which have the actual URLs)
function extractApiEndpoints(requests) {
  const endpoints = new Map();
  
  requests.forEach(req => {
    if (req.type === 'response' && isApiCall(req.url)) {
  ...
  endpoints.forEach(endpoint => {
    try {
      ...
      const item = {
        ...
        response: [{
          name: `Response - ${endpoint.status} ${endpoint.statusText}`,
          originalRequest: {
            method: endpoint.method,
            header: [
              {
          ...
          body: endpoint.body || 'No response body available'
        }]
      };
...
internalEndpoints.forEach(ep => {
  console.log(`  ${ep.method} ${ep.endpoint} (Status: ${ep.status}, used on: ${ep.pages.join(', ')})`);
});
