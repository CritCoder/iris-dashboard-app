...
import { ArrowLeft, MapPin, TrendingUp, MessageCircle, Share2, Eye, Loader2, Users, Globe, Building, Home } from 'lucide-react'
...
export function LocationDetailView({ location, onBack }: LocationDetailViewProps) {
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            <div className="flex items-start gap-3 mb-2">
              ...
              <div className="flex-1">
                <h1 className="text-2xl font-bold text-foreground mb-1">{formatLocationName(location)}</h1>
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {location.riskLevel && location.riskLevel !== 'LOW' && (
                    <Badge 
                      variant={location.riskLevel === 'HIGH' ? 'destructive' : 'default'}
                      className="gap-1"
                    >
            ...
            <div className="flex items-center gap-4 text-sm text-muted-foreground">
              <span>
                <strong className="text-foreground">{location.totalMentions?.toLocaleString() || 0}</strong> mentions
              </span>
              {location.postCount && (
                <span>
                  <strong className="text-foreground">{location.postCount}</strong> posts
                </span>
              )}
              {location.uniqueUsers && (
                <span>
                  <strong className="text-foreground">{location.uniqueUsers}</strong> unique users
                </span>
              )}
              {location.lastSeen && (
                <span>Last seen: <strong className="text-foreground">{location.lastSeen}</strong></span>
              )}
            </div>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="flex items-center justify-center py-12">
            <div className="text-center">
              ...
              <p className="text-muted-foreground">No posts found for this location</p>
            </div>
          </div>
        ) : (
